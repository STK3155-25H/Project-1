% File: ols_mse.tex
\begin{figure}[H]
\begin{tikzpicture}
\begin{axis}[
    width=8cm,
    height=6cm,
    xlabel={Polynomial degree},
    ylabel={MSE},
    title={Cross Validation},
    legend pos=north east,
    grid=both,
    y tick label style={
        /pgf/number format/fixed,
        /pgf/number format/precision=2
    }
    ]
    \addplot [blue, mark=none] table [x=degree, col sep=comma] {../outputs/tables/part_h_cv_mse_ols.csv};
    \addlegendentry{$OLS$}

    \addplot [orange, mark=none] table [x=degree, col sep=comma] {../outputs/tables/part_h_cv_mse_ridge.csv};
    \addlegendentry{$RIDGE$}

    \addplot [green!70!black, mark=none] table [x=degree, col sep=comma] {../outputs/tables/part_h_cv_mse_lasso.csv};
    \addlegendentry{$LASSO$}
\end{axis}
\end{tikzpicture}
\caption{Cross-validation test MSE for OLS, Ridge, and LASSO regression across polynomial degrees. The plot compares model performance as complexity increases, highlighting how regularization in Ridge and LASSO helps control overfitting relative to OLS.}
\label{fig:Cross_Val}
\end{figure}