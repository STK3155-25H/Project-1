...
% Across all methods, the $R^2$ scores and MSE values of the implemented solvers matched those obtained with \texttt{scikit-learn} within numerical tolerance ($\text{rtol}\le 10^{-10}$). 
% This confirms that the custom implementations are both accurate and numerically stable.
% For both OLS and Ridge, the difference in $R^2$ between the custom and scikit-learn implementations was smaller than $10^{-4}$ across all degrees.
% Small deviations in time efficiency are attributable to different BLAS backends and matrix factorization strategies (SVD vs.\ Cholesky).

% \paragraph*{Conditioning and scaling.}
% The conditioning number of $\mathbf{X}^\top\mathbf{X}$ increased exponentially with polynomial degree, reaching $\sim 2\times10^{10}$ at degree~15. 
% This behavior is consistent with the well-known ill-conditioning of high-order polynomial regression.
% Feature scaling significantly improved numerical stability, reducing the relative difference in test MSE between scaled and unscaled inputs by an average of 40\%.
% This demonstrates the necessity of feature normalization before performing polynomial expansions or iterative optimization.
...
% \paragraph*{Summary.}
% The computational study confirms the following hierarchy of efficiency:
% \begin{itemize}
%     \item \textbf{OLS / Ridge}: fastest methods, near-zero iteration cost, ideal for low- to medium-dimensional problems.
%     \item \textbf{Momentum / Adam}: slower but robust iterative methods with efficient convergence.
%     \item \textbf{Vanilla GD}: highest iteration cost and sensitivity to learning rate.
% \end{itemize}
% Scaling and regularization remain essential to maintain numerical stability, especially at higher polynomial degrees.

% \begin{figure}[H]
% \centering
% \input{Graphs/Computational_Pareto.tex}
% \caption{Pareto frontier showing $R^2_{test}$ vs.\ computational time (ms) for each method. 
% \textit{Data source:} columns \texttt{method}, \texttt{fit\_time\_ms}, \texttt{r2\_test} from \texttt{benchmarks\_20251005-140158.csv}. 
% Each point represents the mean across all polynomial degrees, with error bars showing one standard deviation across runs.}
% \label{fig:Pareto_Frontier}
% \end{figure}

% \begin{figure}[H]
% \centering
% \input{Graphs/Conditioning_vs_Degree.tex}
% \caption{Condition number of $\mathbf{X}^\top\mathbf{X}$ as a function of polynomial degree. 
% \textit{Data source:} column \texttt{cond\_XTX}. The rapid exponential growth illustrates the ill-conditioning of high-degree polynomial regressors.}
% \label{fig:Conditioning}
% \end{figure}

% \begin{figure}[H]
% \centering
% \input{Graphs/Scaling_Stability.tex}
% \caption{Difference in test MSE between scaled and unscaled data as a function of degree. 
% \textit{Data source:} column \texttt{scaled\_vs\_unscaled\_mse\_delta}. Negative values indicate improvement from scaling.}
% \label{fig:Scaling_Stability}
% \end{figure}
