...
For the implemented preprocessing routines,  the generated design matrices were compared element-wise with those produced by scikit-learn's 
\texttt{PolynomialFeatures} and \texttt{StandardScaler} classes, verifying identical scaling and intercept behavior. 
Metric functions such as the Mean Squared Error (MSE) and the coefficient of determination ($R^2$) were also cross-checked 
against scikit-learn's corresponding functions, respectively, ensuring numerical equivalence up to machine precision.

Analytical regression solvers like for OLS and Ridge Regression problems were validated by comparing their estimated coefficients 
and predictions to those obtained from scikit-learn functions under identical configurations. 
...
Furthermore, the custom gradient descent optimizer was evaluated by verifying that its converged parameters 
coincide with the analytical OLS solution within a tolerance of $10^{-5}$, demonstrating the correctness of the gradient computations and convergence logic. 

Finally, the bias-variance decomposition implemented was internally validated by checking the identity 
\[
\mathrm{MSE} \approx \mathrm{Bias}^2 + \mathrm{Variance},
\]
using synthetic bootstrap predictions. 
All assertions passed successfully, confirming that the entire computational pipeline, from data scaling to analytical and iterative solvers,
produces results consistent with standard machine learning libraries and theoretical expectations.
