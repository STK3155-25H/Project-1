...
\include{Graphs/FIT_TIME}

% \paragraph*{Gradient descent variants.}
% Among all iterative methods, vanilla GD required the largest number of iterations (median $\sim10^4$) and showed the slowest convergence. 
% Momentum and Adam substantially improved convergence speed and smoothness of the loss curve, maintaining stable $R^2$ scores close to the analytical reference.
% RMSprop and Adagrad performed comparably, though their highly adaptive step sizes occasionally led to slower convergence.
% Overall, Adam achieved the best balance between convergence speed and stability among the iterative methods. 
% This behaviour is illustrated in Figure~\ref{fig:gd_iterations_vs_degree}, showing the number of iterations required to meet the stopping criterion across degrees.

% % \begin{figure}[H]
% %     \centering
% %     \includegraphics[width=.95\columnwidth]{/mnt/data/gd_iterations_vs_degree_20251005-140158.png}
% %     \caption{Iterations to convergence for gradient-based methods as a function of polynomial degree. Adam maintains a stable iteration count, while vanilla GD and momentum require substantially more steps.}
% %     \label{fig:gd_iterations_vs_degree}
% % \end{figure}

% \paragraph*{Conditioning and numerical stability.}
% The conditioning of the design matrix $\mathbf{X}^\top \mathbf{X}$ increases exponentially with polynomial degree, as shown in Figure~\ref{fig:condition_vs_degree}. 
% At degree~15, the condition number reaches $\sim10^{10}$, consistent with the ill-conditioning typical of high-degree polynomial regression.
% This directly explains the rise in variance observed in the bias–variance analysis (Figure~\ref{fig:bias_variance}) and the increased sensitivity of gradient descent at high degrees. 
% Feature scaling proved crucial for mitigating these effects: the stability proxy in Figure~\ref{fig:stability_delta_vs_degree} shows that scaling reduced numerical discrepancies in test MSE to the order of $10^{-12}$ across all degrees, confirming negligible numerical drift.

% % \begin{figure}[H]
% %     \centering
% %     \includegraphics[width=.95\columnwidth]{/mnt/data/condition_vs_degree_20251005-140158.png}
% %     \caption{Condition number of $\mathbf{X}^\top\mathbf{X}$ (log scale) as a function of polynomial degree. Exponential growth confirms the ill-conditioning of high-order polynomial regressors.}
% %     \label{fig:condition_vs_degree}
% % \end{figure}

% % \begin{figure}[H]
% %     \centering
% %     \includegraphics[width=.95\columnwidth]{/mnt/data/stability_delta_vs_degree_20251005-140158.png}
% %     \caption{Numerical stability proxy: $\Delta$ Test MSE between scaled and unscaled inputs. Scaling ensures numerical consistency down to machine precision across all degrees.}
% %     \label{fig:stability_delta_vs_degree}
% % \end{figure}

% \paragraph*{Bias–variance relation and overfitting.}
% The bias–variance decomposition for OLS is reported in Figure~\ref{fig:bias_variance}.
% At low polynomial degrees, the MSE is dominated by bias, indicating underfitting. 
% As the degree increases, bias decreases while variance remains low up to degree~9.
% Beyond this point, the variance term rises sharply, dominating the total error and leading to the observed MSE spikes at degrees~11 and~13.
% This behaviour clearly illustrates the classical bias–variance trade–off: increasing model flexibility initially improves accuracy, but excessive complexity causes instability and overfitting.

% \begin{figure}[H]
%     \centering
%     \includegraphics[width=.95\columnwidth]{/mnt/data/bias_variance_20251005-140158.png}
%     \caption{Bias–variance decomposition for OLS. The rapid variance growth at high degrees reflects overfitting due to ill-conditioning of $\mathbf{X}^\top\mathbf{X}$.}
%     \label{fig:bias_variance}
% \end{figure}

% \paragraph*{Accuracy trends across degrees.}
% Figure~\ref{fig:r2_test_vs_degree} shows that all models improve $R^2$ with increasing degree until saturation around degree~10.
% OLS reaches nearly perfect fits ($R^2 \approx 1.0$), while Ridge stabilizes slightly below due to its regularization term.
% Among iterative methods, Adam achieves the highest accuracy, followed by momentum and vanilla GD.
% This pattern confirms that adaptive optimizers mitigate gradient explosion and preserve generalization even when analytical conditioning deteriorates.

% \begin{figure}[H]
%     \centering
%     \includegraphics[width=.95\columnwidth]{/mnt/data/r2_test_vs_degree_20251005-140158.png}
%     \caption{Test $R^2$ as a function of polynomial degree. Analytical solvers outperform gradient-based methods, but Adam remains competitive among iterative optimizers.}
%     \label{fig:r2_test_vs_degree}
% \end{figure}
...
\include{Graphs/FLOPS}
\include{Graphs/Memory}
...
% \include{Graphs/PARETO}
\include{Graphs/PARETO2}
...
The study demonstrated the value of regularization and adaptive optimization in regression analysis.
Limitations include the focus on a single dataset and reliance on MSE as the main metric.
Future work should extend these methods to real-world and higher-dimensional data, to further enhance model performance and interpretability.
