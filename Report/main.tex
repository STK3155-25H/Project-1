...
\subsection{OLS}
\label{OLS}
The analysis begins with standard OLS regression using polynomials up to degree 15.
Figure \ref{fig:OLS_graph} shows how MSE varies with polynomial degree and dataset size.\\
In general, MSE decreases as the polynomial degree increases.
However, for small datasets ($n=40or50$), MSE rises at higher degrees due to overfitting.
As the model becomes more complex, it fits not only the underlying trend but also the random noise, reducing performance on unseen data.
This effect is less noticeable with larger datasets.\\\\
...
This behavior reflects overfitting: at high degrees, the model captures both the true signal and the noise, causing the coefficients to become unstable.
Using more data points reduces this effect and stabilizes the parameter estimates
    
\input{Graphs/OLS_MSE.tex}

\subsection{Ridge regression}
\label{Ridge regression}
Now, Ridge regression is applied to the same dataset, exploring the impact of different regularization parameters $\lambda$.
First, the effect of the regularization parameter $\lambda $ on the MSE as a function of polynomial degree was analyzed for a fixed number of data points n=100: the results are shown in Figure \ref{fig:RIDGE_MSE_Lambda}.
Small values of $\lambda$ (e.g., 0.001, 0.01) behave similarly to OLS, with the MSE decreasing as polynomial degree increases.
However, as $\lambda$ increases (e.g., 1, 10, 100), the MSE curve flattens and even increases for high polynomial degrees, indicating that strong regularization prevents overfitting but may lead to underfitting if too large.
...
This simplified the model and generally produced slightly lower MSE compared to Ridge due to the stronger bias introduced.\\\\
...
Bootstrap resampling provided a detailed view of the bias–variance trade-off across polynomial degrees.
For low-degree models, the variance was minimal and the MSE closely followed the bias, indicating stable underfitting.
As model complexity increased, variance rose sharply, causing MSE to grow due to overfitting.
The bootstrap analysis also highlighted the effect of dataset size: larger datasets reduced variance and stabilized MSE, allowing higher-degree models to generalize better.\\\\
Cross-validation confirmed these observations by evaluating OLS, Ridge, and Lasso models across polynomial degrees.
OLS exhibited low bias but high variance at high degrees, resulting in overfitting.
Ridge and Lasso introduced regularization, slightly increasing bias but significantly reducing variance, yielding more stable predictions and better generalization.
Lasso additionally promoted sparsity, slightly improving MSE compared to Ridge in some cases.\\\\
Together, the bootstrap and cross-validation analyses illustrate the fundamental **bias–variance trade-off**: the optimal model complexity balances low bias and moderate variance to minimize prediction error, and regularization methods like Ridge and Lasso are effective tools for achieving this balance, particularly when the number of data points is limited.
